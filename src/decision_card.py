...
def _deadline(value: datetime | None) -> str:
    ...
    days = (current_utc - datetime.now(timezone.utc)).total_seconds() / 86400
    return f"{current.astimezone(MSK).strftime('%d.%m.%Y %H:%M')} (осталось {max(0, days):.1f} дн.)"
...
def render_decision_card(
    ...
    lines = [
        ...
        f"Аванс: <b>{'да' if tender.advance_required else 'нет/не указан'}</b>"
        + (f" ({_percent(tender.advance_percent)})" if tender.advance_percent is not None else ""),
        f"Постоплата: <b>{tender.postpayment_days} дн.</b>"
        if tender.postpayment_days is not None
        else "Постоплата: <b>не указана</b>",
    ...
    lines.append(f"<a href=\"{escape(tender.url or '', quote=True)}\">Открыть тендер</a>")
    return "\n".join(lines)
