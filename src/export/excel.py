...
def export_tenders_to_excel(
    ...
    with db._connect() as conn:
        if tender_ids:
            ...
            rows = conn.execute(
                ...
                ORDER BY
                    CASE WHEN a.relevance_score IS NULL THEN 0 ELSE a.relevance_score END DESC,
                    t.published_at DESC
                """,
                tender_ids,
            ).fetchall()
    ...
    for row in rows:
        ...
        recommendation_names = {
            "participate": "Участвовать", "review": "Рассмотреть", "skip": "Не участвовать",
        }
        ...
        if row["risks"]:
            try:
                ...
                if isinstance(parsed_risks, list):
                    risks = "; ".join(str(x).strip() for x in parsed_risks if str(x).strip())
                else:
    ...
    # AutoFilter намеренно не включаем: пользователь просил убрать стрелки
    # фильтра из заголовков. Первую строку при этом оставляем закреплённой.
    ws.freeze_panes = "A2"
    ...
    high_score = sum(
        1 for row in rows
        if row["relevance_score"] is not None and int(row["relevance_score"]) >= 70
    )
