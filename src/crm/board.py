...
class TenderBoard:
    ...
    def _label_key(label: str) -> str:
        return " ".join(str(label).casefold().split()).strip()
    ...
    def add_label(self, tender_id: int, label: str) -> None:
        ...
        with self.db._connect() as conn:
            existing = conn.execute("SELECT label FROM tender_board_labels WHERE tender_id=?", (tender_id,)).fetchall()
            if any(self._label_key(row["label"]) == key for row in existing):
                return
            conn.execute("INSERT INTO tender_board_labels(tender_id,label,created_at) VALUES(?,?,?)", (tender_id, label, now))
    ...
    def remove_label(self, tender_id: int, label: str) -> None:
        ...
        with self.db._connect() as conn:
            existing = conn.execute("SELECT label FROM tender_board_labels WHERE tender_id=?", (tender_id,)).fetchall()
            for row in existing:
                if self._label_key(row["label"]) == key:
                    conn.execute("DELETE FROM tender_board_labels WHERE tender_id=? AND label=?", (tender_id, row["label"]))
    ...
    def upcoming_deadlines(self, days: int = 7) -> list[dict]:
        ...
        lower = int(now.timestamp())
        upper = lower + days * 86400
        with self.db._connect() as conn:
            rows = conn.execute("""
                SELECT b.tender_id,b.status,b.assignee_id,t.platform,t.external_id,t.title,t.price,t.customer,t.region,t.deadline,t.end_date,t.url
                FROM tender_board b JOIN tenders t ON t.id=b.tender_id
                WHERE b.status IN ('new','reviewing','participating','docs_preparation','submitted','waiting_result')
                  ...
                  AND CAST(strftime('%s',COALESCE(t.deadline,t.end_date)) AS INTEGER) BETWEEN ? AND ?
