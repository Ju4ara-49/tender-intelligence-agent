...
ALL_STATUSES: tuple[str, ...] = (STATUS_NEW, STATUS_REVIEWING, STATUS_PARTICIPATING, STATUS_DOCS, STATUS_SUBMITTED, STATUS_WAITING, STATUS_WON, STATUS_LOST, STATUS_SKIPPED)
ACTIVE_STATUSES: frozenset[str] = frozenset({STATUS_REVIEWING, STATUS_PARTICIPATING, STATUS_DOCS, STATUS_SUBMITTED, STATUS_WAITING})
...
class TenderBoard:
    ...
    def add_label(self, tender_id: int, label: str) -> None:
        ...
        with self.db._connect() as conn:
            self._require_tender(conn, tender_id)
            rows = conn.execute("SELECT label FROM tender_labels WHERE tender_id = ?", (tender_id,)).fetchall()
            if any(str(row["label"]).casefold() == normalized.casefold() for row in rows):
                return
            conn.execute("INSERT INTO tender_labels (tender_id, label, created_at) VALUES (?, ?, ?)", (tender_id, normalized, now))
    ...
    def remove_label(self, tender_id: int, label: str) -> None:
        ...
        with self.db._connect() as conn:
            self._require_tender(conn, tender_id)
            rows = conn.execute("SELECT id, label FROM tender_labels WHERE tender_id = ?", (tender_id,)).fetchall()
            target = next((row for row in rows if str(row["label"]).casefold() == normalized.casefold()), None)
            if target is None:
                return
            conn.execute("DELETE FROM tender_labels WHERE id = ?", (target["id"],))
            conn.execute("INSERT INTO tender_board_history (tender_id, changed_at, event_type, old_value) VALUES (?, ?, 'label_removed', ?)", (tender_id, now, str(target["label"])))
