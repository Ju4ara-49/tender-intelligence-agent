...
class ModernB2BCenterCollector(AuthenticatedB2BCenterCollector):
    """Use B2B-Center's current public market-search UI instead of legacy /market/."""
    ...
    def _authenticated_search_html(self, keyword: str) -> str:
        url = self._modern_search_url(keyword)
        return self._authenticated_page_html(url)

    def _load_search_page(self, keyword: str) -> str:
        """Load the current /app/next/market-search page in a browser."""
        try:
            if self.authenticated and self.STORAGE_STATE.exists():
    ...
    def _parse_search_html(self, html: str, keyword: str, since: datetime | None = None) -> list[Tender]:
        """Parse current React/Next result cards without relying on old table markup."""
        ...
        for link in anchors:
            ...
            # A procedure link normally contains a long numeric ID or a tender/procedure path.
            if not self._looks_like_procedure_url(href):
                continue
            ...
            if since is not None and published is not None:
                if self._normalize_datetime(published) < self._normalize_datetime(since):
                    continue
            ...
            if len(results) >= self.max_pages * 20:
                break
    ...
    def _looks_like_procedure_url(href: str) -> bool:
        low = href.lower()
        if any(x in low for x in ("tender", "procedure", "purchase", "trade", "market")):
            return True
        return bool(re.search(r"\d{7,}", href))
    ...
    def _result_container(link):
        ...
        for _ in range(7):
            ...
            role = node.get("role", "") if getattr(node, "get", None) else ""
    ...
    def _extract_date_from_text(cls, text: str, published: bool) -> datetime | None:
        ...
        if not match:
            return None
        return cls._parse_date_text(match.group(1))
    ...
    def search(self, keywords: list[str], since: datetime | None = None) -> list[Tender]:
        # Keep the normal orchestrator contract but remove the inherited early stop at 100
        # by using the configured max_results explicitly for the modern search.
        return super().search(keywords, since=since)
