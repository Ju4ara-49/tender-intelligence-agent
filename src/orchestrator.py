...
class Orchestrator:
    ...
    def __init__(self, settings: AppSettings) -> None:
        ...
        self.analyzer = TenderAnalyzer(model=settings.ai_model, ai_context=settings.ai_context, use_stub_when_no_key=settings.ai_use_stub, ollama_url=settings.ollama_url)
        logger.info("AI: provider=%s | model=%s | ollama_url=%s | configured=%s", settings.ai_provider, settings.ai_model, settings.ollama_url, self.analyzer.is_configured)
        self.notifier = TelegramNotifier(bot_token=settings.telegram_bot_token, chat_id=settings.telegram_chat_id, dry_run_when_no_token=settings.telegram_dry_run)
        self.email_notifier = EmailNotifier(enabled=settings.email_enabled, smtp_host=settings.email_smtp_host, smtp_port=settings.email_smtp_port, username=settings.email_from, password=settings.email_password, recipient=settings.email_to)
    ...
    def run_cycle(self) -> dict[str, int]:
        ...
        for collector, tender in all_tenders:
            ...
            existing_tender_id = self.db.get_tender_id(tender.unique_key) if existing else None
            ...
            if criteria.min_price is not None and tender.price is not None and tender.price < criteria.min_price: stats["excluded_by_criteria"] += 1; continue
            if criteria.max_price is not None and tender.price is not None and tender.price > criteria.max_price: stats["excluded_by_criteria"] += 1; continue
            if criteria.min_submission_days and tender.deadline is not None:
                days_left = (tender.deadline - datetime.now(timezone.utc)).days
                if days_left < criteria.min_submission_days: stats["excluded_by_criteria"] += 1; continue
            ...
            if self.db.was_notified(tender.unique_key): stats["skipped_duplicate"] += 1; continue
