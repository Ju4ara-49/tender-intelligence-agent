...
class Orchestrator:
    ...
    def _passes_criteria(tender: Tender, criteria: TenderCriteria) -> tuple[bool, str]:
        if criteria.min_price is not None and (tender.price is None or tender.price < criteria.min_price): return False, "min_price"
        if criteria.max_price is not None and (tender.price is None or tender.price > criteria.max_price): return False, "max_price"
        if criteria.advance_required:
            if not tender.advance_required: return False, "advance_required"
            if tender.advance_percent is None: return False, "advance_percent_missing"
        if criteria.min_advance_percent > 0 and (tender.advance_percent is None or tender.advance_percent < criteria.min_advance_percent): return False, "min_advance_percent"
        if criteria.max_postpayment_days is not None and (tender.postpayment_days is None or tender.postpayment_days > criteria.max_postpayment_days): return False, "max_postpayment_days"
        if criteria.min_application_security_percent > 0 and (tender.application_security_percent is None or tender.application_security_percent < criteria.min_application_security_percent): return False, "min_application_security_percent"
        if criteria.max_application_security_percent is not None and (tender.application_security_percent is None or tender.application_security_percent > criteria.max_application_security_percent): return False, "max_application_security_percent"
        if criteria.min_contract_security_percent > 0 and (tender.contract_security_percent is None or tender.contract_security_percent < criteria.min_contract_security_percent): return False, "min_contract_security_percent"
        if criteria.max_contract_security_percent is not None and (tender.contract_security_percent is None or tender.contract_security_percent > criteria.max_contract_security_percent): return False, "max_contract_security_percent"
        if criteria.min_submission_days and tender.deadline is not None:
            if (tender.deadline - datetime.now(timezone.utc)).total_seconds() < criteria.min_submission_days * 86400: return False, "min_submission_days"
    ...
    def _passes_regions(tender: Tender, regions: list[str] | None) -> bool:
        ...
        if not tender_region: return False
        return any(region.strip().casefold() in tender_region for region in regions if region.strip())

    def run_cycle(
        self, user_id: str | int | None = None, criteria: TenderCriteria | None = None,
        keywords: list[str] | None = None, platforms: list[str] | None = None,
        exclude_keywords: list[str] | None = None, regions: list[str] | None = None,
        notification_recipient_key: str | None = None, notification_chat_id: str | None = None,
    ) -> dict[str, int]:
        search_number = self._get_next_search_number()
        stats = {"search_number": search_number, "found": 0, "soft_filtered": 0, "filtered": 0, "keyword_excluded": 0, "new": 0, "analyzed": 0, "notified": 0, "skipped_duplicate": 0, "excluded_by_criteria": 0, "excluded_by_region": 0, "details_loaded": 0, "details_partial": 0, "details_failed": 0, "saved": 0, "ai_failed": 0}
        self.clear_stop_request(); self.last_run_results = []
        ...
        recipient_key = str(notification_recipient_key).strip() if notification_recipient_key else (f"user:{str(user_id).strip()}" if user_id is not None and str(user_id).strip() else NotificationDeliveryState.DEFAULT_RECIPIENT_KEY)
        target_chat_id = str(notification_chat_id).strip() if notification_chat_id else (str(user_id).strip() if user_id is not None and str(user_id).strip() else None)
        ...
        workers = self._platform_worker_count(self.settings.config.get("search", {}), len(collectors))
        with ThreadPoolExecutor(max_workers=workers, thread_name_prefix="collector") as pool:
            ...
            for future in as_completed(futures):
                platform, found = future.result(); collector = next((c for c in collectors if c.platform == platform), None)
                if collector is not None: all_pairs.extend((collector, tender) for tender in found)
        stats["found"] = len(all_pairs)
        unique_pairs = self._deduplicate_pairs(all_pairs); now = datetime.now(timezone.utc); soft_pairs = []
        for collector, tender in unique_pairs:
            if self.stop_requested: break
            config = self.settings.config.get("collectors", {}).get(collector.platform, {})
            since = now - timedelta(days=int(config.get("lookback_days", 3))); published = self._normalize_datetime(tender.published_at)
            if published is not None and published < since: continue
            tender = self._normalize_tender_datetimes(tender)
            if self.keyword_filter.matches_soft(tender): soft_pairs.append((collector, tender))
        stats["soft_filtered"] = len(soft_pairs)
        enriched_pairs = []
        for collector, tender in soft_pairs:
            if self.stop_requested: break
            enriched, detail_loaded = self._enrich_tender(collector, tender); detail_status = str(getattr(enriched, "detail_status", "partial") or "partial").lower()
            stats["details_loaded"] += int(detail_status == "success" and detail_loaded); stats["details_partial"] += int(detail_status == "partial"); stats["details_failed"] += int(detail_status == "failed" or not detail_loaded and detail_status == "failed")
            enriched_pairs.append((collector, enriched)); existing = self.db.exists(enriched.unique_key); self.db.save_tender(enriched); stats["saved"] += 1
            if not existing: stats["new"] += 1
        strict_pairs = []
        for collector, tender in enriched_pairs:
            if tender.detail_status == "failed": continue
            if self.keyword_filter.matches_strict(tender): strict_pairs.append((collector, tender))
            else: stats["keyword_excluded"] += 1
        stats["filtered"] = len(strict_pairs)
        export_tender_ids: list[int] = []
        for collector, tender in strict_pairs:
            if self.stop_requested: break
            if not self._passes_regions(tender, selected_regions): stats["excluded_by_region"] += 1; continue
            passed, reason = self._passes_criteria(tender, criteria)
            if not passed: stats["excluded_by_criteria"] += 1; logger.debug("Критерии: исключён %s:%s (%s)", tender.platform, tender.external_id, reason); continue
            self.last_run_results.append(tender); tender_id = self.db.get_tender_id(tender.unique_key)
            if tender_id is None: logger.error("Tender disappeared after save: %s", tender.unique_key); continue
            export_tender_ids.append(tender_id)
            if self.notification_state.was_notified(tender, recipient_key=recipient_key): stats["skipped_duplicate"] += 1; continue
            try: analysis = self.analyzer.analyze(tender)
            except Exception as exc: stats["ai_failed"] += 1; logger.exception("AI: ошибка анализа %s: %s", tender.unique_key, exc); continue
            self.db.save_analysis(tender_id, analysis); stats["analyzed"] += 1
            if analysis.relevance_score < criteria.min_ai_score: continue
            if self.notification_state.was_notified(tender, recipient_key=recipient_key): stats["skipped_duplicate"] += 1; continue
            if self.notifier.send_tender_alert(tender, analysis, chat_id=target_chat_id): self.notification_state.mark_notified(tender, recipient_key=recipient_key); stats["notified"] += 1
        try:
            output_dir = Path(self.settings.config.get("export", {}).get("output_dir", "output")); output_dir.mkdir(parents=True, exist_ok=True)
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S"); excel_path = output_dir / f"search_{search_number:03d}_{timestamp}.xlsx"
            export_path = export_tenders_to_excel(self.db, excel_path, tender_ids=export_tender_ids, search_number=search_number)
            logger.info("Excel: создан новый файл текущего прогона: %s", export_path); self.email_notifier.send_excel(export_path, search_number)
        except Exception: logger.exception("Excel: ошибка экспорта результатов")
        return stats
