...
## Validation cycle 14.09.2026 — TenderPlan-style web search form

По запросу пользователя добавлен локальный браузерный интерфейс настройки SearchProfile, визуально ориентированный на предоставленный пример ТендерПлан: тёмная боковая навигация, верхние вкладки, крупный заголовок, сгруппированные панели критериев, диапазоны цены/обеспечений, раскрывающиеся дополнительные условия и явная кнопка сохранения.

Ключевое архитектурное решение: web UI не создаёт отдельный формат настроек. POST формы преобразуется непосредственно в канонический `SearchProfile`, после чего сохраняется через существующий `SearchProfileStore`. Поддержаны название, ключевые слова, исключения, регионы, площадки, цена, аванс, постоплата, срок до подачи, обеспечение заявки/контракта и AI score. Повторяющиеся checkbox values корректно собираются; значения экранируются перед HTML-рендерингом. Сервер по умолчанию слушает только 127.0.0.1.

Добавлена команда `python -m src.main web` и документация README. Добавлены regression tests на преобразование формы, multi-select, defaults и HTML escaping.

Первый CI #603 выявил реальный дефект новой формы: `parse_qs()` возвращает несколько значений checkbox как список, а первоначальный helper брал только первый. Исправлено в `7dce5caf5c4b55070d49872d3fa40ed75000b829`.

CI #604 — SUCCESS на `7dce5caf5c4b55070d49872d3fa40ed75000b829`.
Browser diagnostics #350 — SUCCESS на том же commit; `ci_failures=[]`.

После документационного commit `af9daff96f66b9df8ea62393401edcb8759b4ef1`: CI #605 — SUCCESS; Browser diagnostics #351 — SUCCESS, `ci_failures=[]`. Реальный browser probe подтвердил B2B-Center (HTTP 200, 23 459 результатов), Фабрикант 223 (HTTP 200), Фабрикант 44 (HTTP 200, 13 результатов), Росатом (HTTP 200, 10 результатов). ЕИС/РТС-Тендер/ТМК снова дали external_timeout — внешнее ограничение runner, не внутренний failure.

Следующая контрольная точка: `af9daff96f66b9df8ea62393401edcb8759b4ef1`.
