...
class BrowserPublicParserTests(unittest.TestCase):
    ...
    def test_browser_detail_extracts_downloadable_documents(self) -> None:
        ...
        """
        tender = RtsTenderCollector()._parse_detail(html, "1234567", "https://223.rts-tender.ru/procedure/1234567/")
        self.assertEqual(len(tender.documents), 2)
        self.assertEqual(tender.documents[0]["url"], "https://223.rts-tender.ru/files/terms.pdf")
    ...
    def test_reliable_browser_rejects_unrelated_global_listing(self) -> None:
        ...
        from src.models.tender import Tender
        tender = Tender(platform="fabrikant", external_id="1", title="Игра для сюжетно-ролевой игры", url="/1", description="Поставка игрушек")
        self.assertFalse(ReliableBrowserSearchMixin._tender_matches_query(tender, "станок"))
